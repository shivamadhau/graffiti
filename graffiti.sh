...
link_modules_raw="https://raw.githubusercontent.com/shivamadhau/phisher-modules"
...
banner(){
	...
	echo -e "${MAGENTA}|  ${CYAN} ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝     ╚═╝   ╚═╝   ╚═╝ ${MAGENTA}|${NC}"
	echo -e "${MAGENTA}-------------------------------------------------------------${NC}"
	echo -e "${BOLDBLUE}           ᴠᴇʀsɪᴏɴ 1.0               𝙱𝚈  -  𝚂𝙷𝙸𝚅𝙰𝙼 𝙰𝙳𝙷𝙰𝚄         ${NC}"
        echo -e ""
}
# Credits banner
cbanner(){
...
sbanner(){
	echo -e " "
        echo -e "${CYAN} ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ ${NC}"
        echo -e "${CYAN} ░/ ___|░░░░░░░░░░░░░  _|░  _|░_)░ |░░░_)░ ${NC}"
	echo -e "${CYAN} ░ |░░░░░  __|  _  |░ |░░░ |░░░ |░ __|░ |░ ${NC}"
        echo -e "${CYAN} ░ |░░ |░ |░░  (_| |░ __|░ __|░ |░ |░░░ |░ ${NC}"
	echo -e "${CYAN} ░\____|░_|░░ \__,_|░_|░░░_|░░░_|░\__|░_|░ ${NC}"
	echo -e "${CYAN} ░░░░░░░░░░░MADE BY SHIVAMADHAU░░░░░░░░░░░ ${NC}"
	echo -e "${NC} "                                           
}
...
directories(){
        ...
        if [[ ! -e "${dgf_dir}/dgf.sh" ]]; then
                wget --no-check-certificate ${link_graffiti_raw}/assets/dgf/dgf.sh > /dev/null 2>&1
                mv dgf.sh ${dgf_dir}
        ...
        if [[ ! -e "${dumps_dir}/line.txt" ]]; then
                wget --no-check-certificate ${link_graffiti_raw}/assets/dumps/line.txt > /dev/null 2>&1
                mv line.txt ${dumps_dir}
                echo -e "\n${BLUE}[${WHITE}+${BLUE}]${GREEN} Downloaded line.txt"
        fi
        if [[ ! -e "${dumps_dir}/space.txt" ]]; then
                wget --no-check-certificate ${link_graffiti_raw}/assets/dumps/space.txt > /dev/null 2>&1
                mv space.txt ${dumps_dir}
                echo -e "\n${BLUE}[${WHITE}+${BLUE}]${GREEN} Downloaded space.txt"
        fi
        if [[ ! -e "${update_dir}/version.txt" ]]; then
                wget --no-check-certificate ${link_graffiti_raw}/assets/update/version.txt > /dev/null 2>&1
        ...
        if [[ ! -e "${email_dir}/sendEmail.sh" ]]; then
                wget --no-check-certificate ${link_graffiti_raw}/assets/email/sendEmail.sh > /dev/null 2>&1
                mv sendEmail.sh ${email_dir}/
                echo -e "\n${BLUE}[${WHITE}+${BLUE}]${GREEN} Downloaded sendEmail.sh"
        fi
        if [[ ! -e "${email_dir}/template.json" ]]; then
                wget --no-check-certificate ${link_graffiti_raw}/assets/email/template.json > /dev/null 2>&1
